...
const libraryCount = db.prepare('SELECT COUNT(*) AS count FROM lang_db').get() as
  | { count: number }
  | undefined;
...
const connectionCount = db.prepare('SELECT COUNT(*) AS count FROM connections').get() as
  | { count: number }
  | undefined;
...
export const DEFAULT_TARGET = 'zh-TW';
